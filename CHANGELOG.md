...
- Improved: WebUI — a full visual refresh: blue-black night palette, a serif display face (Fraunces) for the wordmark and card titles, monospaced sizes/timestamps/log (IBM Plex Mono), and outlined status chips. The save button now lives in a bar that appears only when there are unsaved changes and names what it saves ("Save “kobo” Profile"); download buttons no longer borrow the success green; conversions from a profile are tagged in the status table. Fonts ship with the image — nothing loads from the internet
